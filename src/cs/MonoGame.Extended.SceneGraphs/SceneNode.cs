...
namespace MonoGame.Extended.SceneGraphs
{
    ...
    public class SpriteEntity : SceneEntity, ISpriteBatchDrawable
    {
        ...
        public float LayerDepth => _sprite.Depth;
    }
    ...
    public class SceneNode : Transform2
    {
        ...
        public void Draw(SpriteBatch spriteBatch, Color? color = null, float layerDepth = 0)
        {
            foreach (var drawable in Entities.OfType<ISpriteBatchDrawable>())
            {
                if (drawable.IsVisible)
                {
                    ...
                    var depth = drawable.LayerDepth;

                    spriteBatch.Draw(texture, position, sourceRectangle, color ?? drawable.Color, -rotation, drawable.Origin, scale, drawable.Effect, depth + layerDepth);
                }
            }

            foreach (var child in Children)
                child.Draw(spriteBatch, color: color, layerDepth: layerDepth);
        }
